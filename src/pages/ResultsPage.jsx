...
import {
  ...
  getSkillCategoryLabels,
} from "../lib/analysis";
import {
  getEntryById,
  getHistoryMeta,
  getHistoryWarningMessage,
  getLatestEntry,
  updateHistoryEntry,
} from "../lib/history";
...
function buildPlanText(entry) {
  return entry.plan7Days
    .map((dayBlock) => `${dayBlock.day} (${dayBlock.focus})\n${dayBlock.tasks.map((task) => `- ${task}`).join("\n")}`)
    .join("\n\n");
}
...
function buildFullExportText(entry) {
  return [
    ...
    `Typical Hiring Focus: ${entry.companyIntel?.typicalHiringFocus || "Practical stack depth"}`,
...
function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const companyIntel = loadedEntry.companyIntel || generateCompanyIntel({ company: loadedEntry.company, jdText: loadedEntry.jdText });
    const roundMapping =
      loadedEntry.roundMapping?.length > 0
    ...
    const expectedFinal = computeFinalScore(normalized.baseScore, normalized.skillConfidenceMap);
    const finalEntry = {
      ...normalized,
    ...
    if (
      ...
      !loadedEntry.roundMapping?.length ||
      loadedEntry.finalScore !== finalEntry.finalScore
    ) {
  ...
  const weakSkills = useMemo(() => {
    if (!entry) return [];
    return getAllSkillsFromExtracted(entry.extractedSkills)
  ...
  const handleSkillChange = (skill, value) => {
    ...
    const updatedMap = { ...entry.skillConfidenceMap, [skill]: value };
    const updatedEntry = {
      ...entry,
      skillConfidenceMap: updatedMap,
      finalScore: computeFinalScore(entry.baseScore, updatedMap),
  ...
  const handleDownload = () => {
    if (!entry) return;
    const text = buildFullExportText(entry);
      ...
      <div className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Run analysis from Home or Practice to generate your first results entry.</CardDescription>
          </CardHeader>
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <Card>
        ...
        <CardContent className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-slate-600">Created: {new Date(entry.createdAt).toLocaleString()}</p>
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700">
              Base: {entry.baseScore}/100
            </span>
            <span className="rounded-full bg-primary/10 px-4 py-2 text-sm font-semibold text-primary">
              Final: {entry.finalScore}/100
            </span>
          </div>
        </CardContent>
      </Card>
            ...
            {entry.roundMapping.map((round, index) => (
              <div key={round.roundTitle} className="relative pl-8">
                ...
                <div className="rounded-xl border border-slate-200 p-4">
                  ...
                  <p className="mt-1 text-sm text-primary">{round.focusAreas.join(" · ")}</p>
                  <p className="mt-2 text-sm text-slate-600">
                    <span className="font-semibold text-slate-700">Why this round matters:</span> {round.whyItMatters}
                  </p>
                </div>
              </div>
            ))}
          ...
          <CardContent className="space-y-4">
            {entry.checklist.map((round) => (
              <div key={round.roundTitle}>
                <p className="text-sm font-semibold text-slate-800">{round.roundTitle}</p>
